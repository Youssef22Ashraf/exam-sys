...
- Never paste a real SMTP password, admin password or production `JWT_SECRET` into a tracked file -- README and compose included. There are no default credentials in the code: admin accounts come from `ADMIN_INITIAL_PASSWORD`, and production refuses to boot on a `JWT_SECRET` that has appeared in this repo.
